 ...
 * 
 */
package org.elastichsearch.ik.core;

import static org.elastichsearch.ik.dic.Dictionary.matchInMainDictWithHit;
import static org.elastichsearch.ik.dic.Dictionary.matchInQuantifierDict;
...
import org.elastichsearch.ik.dic.Hit;
		...
		if(CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()){
			...
			if(!this.countHits.isEmpty()){
				...
				for(Hit hit : tmpArray){
					hit = matchInMainDictWithHit(context.getSegmentBuff(), context.getCursor() , hit);
					if(hit.isMatch()){
						//输出当前的词
			...
			//*********************************
			//对当前指针位置的字符进行单字匹配
			Hit singleCharHit =matchInQuantifierDict(context.getSegmentBuff(), context.getCursor(), 1);
